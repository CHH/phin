...
class Response
{
    ...
    static function fromArray(array $response)
    {
        ...
        return new static($status, $headers, $response);
    }
    ...
    function toString()
    {
        $message = $this->headersToString();
        $body = $this->body;
        
        if (is_string($body) or is_callable(array($body, "__toString"))) {
            $message .= $body;
            
        } else if (is_array($body) or $body instanceof \Traversable) {
            foreach ($body as $line) {
                $message .= $line;
            }
            
        } else if (is_resource($body)) {
            $message .= stream_get_contents($body);
            fclose($body);
        }
        unset($body);
        return $message;
